...
public class GoodsController {
	...
	public Result update(@RequestBody TbGoods goods){
		//校验是否当前商家ID
		// 由页面商品id获得数据库实体
		TbGoods tbGoods = goodsService.findOne(goods.getId());
		//获取当前登陆的商家ID
		String loginSellerId = SecurityContextHolder.getContext().getAuthentication().getName();
		//如果传递来的商家ID不是当前登陆的用户ID，则属于非法操作
		if (!tbGoods.getSellerId().equals(loginSellerId) || !goods.getSellerId().equals(loginSellerId)){
